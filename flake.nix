{
  ...
  inputs = {
    ...
    sipyco = { url = github:m-labs/sipyco; inputs.nixpkgs.follows = "nixpkgs"; };
    src-pythonparser = { url = github:m-labs/pythonparser; flake = false; };
    artiq-comtools = {
  ...
  outputs = { self, nixpkgs, rust-overlay, sipyco, src-pythonparser, artiq-comtools, src-migen, src-misoc, pre-commit-hooks }:
    let
      ...
      rustBin = pkgs.rust-bin.nightly."2021-08-31".default.override {
        extensions = [ "rust-src" ];
        targets = [ ];
      };
      rust = rustBin;
    ...
    rec {
      ...
      packages.x86_64-linux.default = pkgs.python3.withPackages (ps: [ packages.x86_64-linux.artiq ]);
